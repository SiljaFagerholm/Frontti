...
@Injectable()
export class RavintolaKommentitService {
  Ravintolat = []
  private actionUrl: string;
  private headers: Headers;
  constructor(private _http: Http) {
    this.actionUrl = 'http://localhost:8080/api/RavintolaKommentit';
...
// }
  ...
  public getRavintolaKommentitRest():Promise<RavintolaKommentti[]>{
    return this._http.get(this.actionUrl)
                  .toPromise()
                  .then(response => response.json() as RavintolaKommentti[])
                  .catch(this.handleError);
   }
   public getRavintolaKommentit(Id:number):Promise<RavintolaKommentti>{
    return this._http.get(this.actionUrl+"/"+Id)
                  .toPromise()
                  .then(response => response.json() as RavintolaKommentti)
                  .catch(this.handleError);
   }
  public createRavintolaKommentit(Nimi:string): Promise<RavintolaKommentti> {
       return this._http.post(this.actionUrl, JSON.stringify({Nimi: name}), {headers: this.headers})
         .toPromise()
         .then(res => res.json() as RavintolaKommentti)
         .catch(this.handleError);
     }
